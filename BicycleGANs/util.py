...
def discriminator_loss(logits_real, logits_fake):
    ...
    for i in range(len(logits_fake)):
        labels = torch.ones(logits_real[i].size()).to(device) #label used to indicate whether it's real or not

        loss_real = nn.MSELoss()(logits_real[i], labels) #real data
        loss_fake = nn.MSELoss()(logits_fake[i], 1 - labels) #fake data

        loss += (loss_real + loss_fake)

    return loss.to(device) * 0.5
...
def generator_loss(logits_fake):
    ...
    for i in range(len(logits_fake)):
        ...
        labels = torch.ones(logits_fake[i].size()).to(device)
        loss += nn.MSELoss()(logits_fake[i], labels)
