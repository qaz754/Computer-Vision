...
class Pix2Pix_AB_Dataloader(Dataset):
    ...
    def __init__(self, img_folder, target_folder, transform=None, shuffle = True, size =256, randomcrop = 224, hflip=0.5, vflip=0.5):
        '''
    ...
    def __getitem__(self, index):
        ...
        if self.shuffle:
            '''
            if shuffle is set to False, assuming the names are the same for the files in the 2 folders, 
            we have a paired dataset
            '''

            index = np.random.randint(0, len(self.target_names))
