...
class Pix2Pix_AB_Dataloader(Dataset):
    ...
    def __len__(self):
        ...
        return max(len(self.file_names), len(self.target_names))

    def __getitem__(self, index):

        left_image = Image.open(self.file_names[index % self.file_len]).convert('RGB')
        right_image = Image.open(self.target_names[index % self.target_len]).convert('RGB')
