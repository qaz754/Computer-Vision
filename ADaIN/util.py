...
def discriminator_loss(logits_real, logits_fake):
    ...
    ones = torch.ones(logits_real.size()).to(device) #label used to indicate whether it's real or not
    zeroes = torch.zeros(logits_fake.size()).to(device)

    loss_real = nn.MSELoss()(logits_real, ones) #real data
    loss_fake = nn.MSELoss()(logits_fake, zeroes) #fake data
...
def KL_Loss(mu, sigma=1):

    if sigma == 1:
        KL_loss = torch.mean(mu ** 2)
    else :
        KL_loss = torch.sum(1 + sigma - mu ** 2 - sigma.exp()) / 2

    return KL_loss

def Style_Loss(transformed, original, cal_function):

    loss = torch.zeros(len(transformed)).to(device)

    for i in range(len(transformed)):

        transformed_mean, transformed_std = cal_function(transformed[i])
        original_mean, original_std = cal_function(original[i])
